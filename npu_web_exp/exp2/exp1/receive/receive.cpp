...
u_int8_t accept_dest_mac[2][6] = { { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF }, { 0xB4, 0xA9, 0xFC, 0x9C, 0x7B, 0xBB } };// B4-A9-FC-9C-7B-BB
...
void ethernet_protocol_packet_callback(u_char* argument, const struct pcap_pkthdr* packet_header, const u_char* packet_content)//数据接收回调函数
{
	...
	//进行长度校验，有两种方法：
	//方法一：
	datalen = len - 4 - sizeof(struct ethernet_header);
	...
	printf("packet header length: %d\n", sizeof(struct ethernet_header));//数据帧爆头长度
	printf("packet data length: %d\n", len - 4 - sizeof(struct ethernet_header));//数据帧数据部分长度
	...
	if (recvfile != NULL)
	{
		...
		for (p = (u_int8_t*)(packet_content + sizeof(struct ethernet_header)); p != (u_int8_t*)(packet_content + packet_header->len - 4); p++)
		{
...
int main()
{
	...
	char pcap_string[] = "rpcap://";
	int id;
	int adapter_id;

	generate_crc32_table();
	recvfile = fopen("recv.txt", "w");
	printf("本机中mac地址列表如下：\n");
	if (pcap_findalldevs_ex(pcap_string, NULL, &all_adapters, error_buffer) == -1)
	{
	...
	*/
	...
	pcap_loop(adapter_handle, 1, ethernet_protocol_packet_callback, NULL);
